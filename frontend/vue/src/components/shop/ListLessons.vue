  ...
  computed: {
    ...
    filteredLessons() {
      ...
      const filtered = this.data.filter((lesson) => {
        ...
        const sportFilter = this.filters.sport && this.filters.sport.length > 0 ? parseInt(this.filters.sport[0]) : null;
        const matchesSport = sportFilter !== null && lesson.idSport !== null
          ? lesson.idSport === sportFilter
          : true;
        // console.log('Lesson:', lesson, 'Sport Filter:', sportFilter, 'Matches Sport:', matchesSport);
        return matchesSport;
      });
      // console.log('Filtered lessons:', filtered);
      return filtered;
    ...
    paginatedLessons() {
      ...
      const paginated = this.filteredLessons.slice(start, end);
      // console.log('Paginated lessons:', paginated);
      return paginated;
    },
  },
  methods: {
    goToDetails(id) {
      console.log('Navigating to lesson details with ID:', id);
